...
#Fig. 3e
obj <- xeno_5genotype
## 1) Drop 3R (keep only 4R conditions)
...
obj4r$Condition <- factor(as.character(obj4r$Condition), levels = keep_levels)


obj4r$broad_celltype <- ifelse(
  as.character(obj4r$control_superclass) %in% c("EN", "IN", "Neu_Early"),
  "Neuron",
  as.character(obj4r$control_superclass)
)

# Make it a factor with proper level order if desired
obj4r$broad_celltype <- factor(
  obj4r$broad_celltype,
  levels = c("Neuron","AST","MG", "NPC_RG", "VLMC")
)

# Check result
table(obj4r$broad_celltype)
...
DimPlot(
  ...
  label = TRUE,
  cols = c("Neuron" = "skyblue") # set Neuron color
) +
  NoAxes() +
  ggtitle("") +
  scale_color_manual(
    values = c(
      "Neuron" = "skyblue",
      setNames(rep("gray80", length(setdiff(unique(obj_no3r$broad_celltype), "Neuron"))),
               setdiff(unique(obj_no3r$broad_celltype), "Neuron"))
    )
  )
...
## Fig 3g: pathfindR KEGG term–gene network + export upregulated genes
